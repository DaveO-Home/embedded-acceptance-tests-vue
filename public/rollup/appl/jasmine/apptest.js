...
export default function (App, dodexApp, login, sidebar, content, footer) {
    describe("Application Unit test suite - AppTest", () => {
        ...
        beforeAll(() => {
            // Add html to karma page for visual output
            $("body").prepend(layout());
        ...
        it("Is Tools Table Loaded", done => {
            ...
            new Promise((resolve, reject) => {
                ...
                const numbers = timer(50, 20);
                const observable = numbers.subscribe(timer => {
                    if (document.querySelector("#data").children.length > 3 || timer === 20) {
                        observable.unsubscribe();
                        expect(App.controllers["Table"]).not.toBeUndefined();
                        expect(document.querySelector("#data").children.length > 3).toBe(true);
                        domTest("tools");
                        done();
                    }
                });
        ...
        cards: {
            card28: {
                ...
                front: {
					...
					<address style="width:385px">/ Add virtual dom to karma page
